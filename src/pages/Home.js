...
class Home extends Component {
  ...
  addTodo = (todo) => {
    ...
    // First, check if the current item already exists in the todo list
    const exists = (this.state.todos).find(t => t.content === todo.content); // Utilize the JS find function
    console.log(exists);
    if (exists !== undefined) {
      return;
    } else { // If the current item doesn't exist, add it to the todo list
      todo.id = Math.random();
